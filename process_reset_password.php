...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['token'], $_POST['password'], $_POST['confirm_password'])) {
    ...
    if ($password === $confirm_password) {
        ...
        $query = "SELECT * FROM users_collection WHERE reset_token = ? AND reset_expiry > NOW() LIMIT 1";
        ...
        if ($result->num_rows > 0) {
            $updateQuery = "UPDATE users_collection SET password = ?, reset_token = NULL, reset_expiry = NULL WHERE reset_token = ?";
